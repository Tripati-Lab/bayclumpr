...
```{r message=FALSE, warning=FALSE, error =FALSE}
PredsBay <- rec.bayesian(calModel = BayesCal$BLM1_fit_NoErrors, recData = recData, iter = 10000, postcalsamples = 100)
```
...
Alternatively, we could explicitly account for uncertainty in the measured $delta_{47}$ performing reconstructions:


```{r message=FALSE, warning=FALSE, error =FALSE}
PredsBay.alt <- rec.bayesian.alt(calModel = BayesCal$BLM1_fit_NoErrors, recData = recData, iter = 20000, postcalsamples = 100)
```

Results for the same model are as follow,
